...
| [`seneca`](skills/seneca/SKILL.md) | Write any human-read text in the voice of Seneca the Younger, in both style and substance. A global, always-on lens applied to docs, READMEs, blog posts, letters, release notes, and posts alike. Enforces hard style rules (no em dashes, no AI slop) and a measured, aphoristic register. Opinion content argues from Stoic ethics (virtue, fortune, what is in your power). Skips machine-facing text (LLM prompts, code, config, commit messages). |
