...
| [`seneca`](skills/seneca/SKILL.md) | Write any human-read text in Graham's voice with a Seneca the Younger Stoic overlay, in both style and substance. The global, always-on sibling of write-like-graham: applies the lens to docs, READMEs, blog posts, letters, release notes, and posts alike. Enforces the same hard rules (no em dashes, no AI slop) and adds a measured, aphoristic register; opinion content argues from Stoic ethics (virtue, fortune, what is in your power) rather than contemporary politics. Skips machine-facing text (LLM prompts, code, config, commit messages). |
