...
| [`seneca`](skills/seneca/SKILL.md) | Write any human-read text in Graham's voice with a Seneca the Younger Stoic overlay. The global, always-on sibling of write-like-graham: applies the lens to docs, READMEs, blog posts, letters, release notes, and posts alike. Enforces the same hard rules (no em dashes, no AI slop) and adds a measured, aphoristic register. Skips machine-facing text (LLM prompts, code, config, commit messages). |
